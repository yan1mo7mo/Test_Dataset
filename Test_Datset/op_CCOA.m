function [GlobalParams] =op_CCOA(data,eLag,eDim)
%% ------------------------------------------------------------------------
...
m=[0.000001,0.000001,0,300,3;
    1,1,1,500,10]; 
global Initial_Value_1 Initial_Value_2 FOBJ;
Initial_Value_1=0.8;
Initial_Value_2=0.3;
global  nfevalMAX lu  Np Nc  D;
D           = 5*(eDim)+1;                    % Problem dimension
% lu          = [0.000001,0.000001,0,300,3;
%    1,1,1,500,10];       % Seach space
lu=repmat(m,1,eDim);% Seach space
lu(1,D)=1;
lu(2,D)=5;
% nfevalMAX   = 20000;                 % Stopping criteria
% Np          = 20;                    % Number of packs
% Nc          = 5;                     % Number of coyotes
nfevalMAX   = 300;                 % Stopping criteria
Np          = 25;                    % Number of packs
Nc          = 7;                     % Number of coyotes
...
chaotic_seq4 = op_addChaos(Initial_Value_1,all_total+1,1);
...
for c=1:pop_total 
    costs(c,1) = FOBJ(coyotes(c,:),data,eLag,eDim);
end
nfeval = pop_total;
%% Output variables
...
while nfeval < nfevalMAX
    ...
    for p=1:n_packs
        ...
        chaotic_seq2(1,:)= op_addChaos(Initial_Value_1,n_coy+1,1);
        chaotic_seq2(2,:)=op_addChaos(Initial_Value_2,n_coy+1,1);
        ...
        for c=1:n_coy_aux
            ...
            new_cost = FOBJ(new_coyotes(c,:),data,eLag,eDim);
        ...
        chaoticNum = op_GetChaoticNum(Initial_Value_1,n_coy,1);
        ...
        pup_cost    = FOBJ(pup,data,eLag,eDim);
        nfeval      = nfeval + 1;
    ...
    if n_packs > 1
        chaoticNum2 = op_GetChaoticNum(Initial_Value_1,n_coy,1);
